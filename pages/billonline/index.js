...
export default function BillOnline({countries, initStates, initCities}) {
  ...
  const handleChangeCountry = async (e) => {
    ...
    if (states && states.length > 0) {
      ...
      setLocationInfo({receiverCountry: country.name, state: states[0].name, city: cities ? (cities.length ? cities[0].name : '') : ''})
    } else {
      let cities = await getCitiesByCountry(country.iso2)
      console.log(cities)
      setStates([])
      setCities(cities ? cities : [])
      setLocationInfo({receiverCountry: country.name, state: '', city: cities ? (cities.length ? cities[0].name : '') : ''})
    }
  }
